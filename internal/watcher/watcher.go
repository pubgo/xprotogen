...
func (t *watcherManager) Start() {
	t.exitFn = xprocess.GoLoop(func(ctx context.Context) (err error) {
		defer xerror.RespErr(&err)
		...
		case event, ok := <-t.watcher.Events:
			...
			if ok {
				...
				case op&fsnotify.Remove == fsnotify.Remove:
					xerror.Panic(t.RemoveRecursive(event.Name))
				case op&fsnotify.Create == fsnotify.Create:
					xerror.Panic(t.AddRecursive(event.Name, fn.(CallBack)))
				}

				if err := fn.(CallBack)(Event{Watcher: t.watcher, Event: event}); err != nil {
					xerror.PanicF(err, event.String())
				}
			}
